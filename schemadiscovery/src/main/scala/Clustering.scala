...
object Clustering {
  ...
  def createPatternsFromLSHClusters(df: DataFrame, propertyDataTypes: Map[String, DataType]): (Array[Pattern], Map[Long, String]) = {
    ...
    val propertyCols = df.columns.filterNot(colName => excludeCols.contains(colName) || colName.endsWith("_typeCode"))

    // Map Spark DataTypes to your type codes using the provided propertyDataTypes
    val propertyTypeCodes: Map[String, Int] = propertyDataTypes.map { case (propName, dataType) =>
      val typeCode = DataTypeMapping.dataTypeToCode.getOrElse(dataType, -1) // Use -1 for unknown types
      propName -> typeCode
    }

    // Broadcast propertyTypeCodes
    val propertyTypeCodesBroadcast = spark.sparkContext.broadcast(propertyTypeCodes)
    ...
    val patterns = clustersAggDF.rdd.map { row =>
      ...
      // Access the broadcasted propertyTypeCodes
      val propertyTypeCodes = propertyTypeCodesBroadcast.value

      // Get the type codes for the common properties
      val commonPropertyTypeCodes = commonProperties.map(prop => prop -> propertyTypeCodes(prop)).toMap

      // Create a Node with the common properties
      val node = Node(
        label = s"Cluster_$hashKey",
        properties = commonProperties.map(prop => prop -> 1).toMap,
        propertyTypeCodes = commonPropertyTypeCodes,
  ...
  def createPatternsFromKMeansClusters(df: DataFrame, propertyDataTypes: Map[String, DataType]): (Array[Pattern], Map[Long, String]) = {
    val spark = df.sparkSession
    import spark.implicits._
    ...
    // Map node IDs to cluster labels
    val nodeIdToClusterLabel = clustersAggDF.select("nodeIds", "clusterLabel")
      .as[(Seq[Long], Int)]
      .flatMap { case (nodeIds, clusterLabel) =>
        val label = s"Cluster_$clusterLabel"
  ...
  // Function to perform incremental LSH clustering
  def performLSHClusteringIncremental(df: DataFrame, increment: Int, propertyDataTypes: Map[String, DataType]): DataFrame = {
    val spark = df.sparkSession
    import spark.implicits._
    ...
    for (i <- 1 to iterations) {
      ...
      val (patterns, nodeIdToClusterLabel) = createPatternsFromLSHClusters(lshDF, propertyDataTypes)
  ...
  // Function to perform incremental K-Means clustering
  def performKMeansClusteringIncremental(df: DataFrame, k: Int, increment: Int, propertyDataTypes: Map[String, DataType]): DataFrame = {
    val spark = df.sparkSession
    import spark.implicits._
    ...
    for (i <- 1 to iterations) {
      ...
      // Print intermediate results
      println(s"\nAfter processing chunk $i/$iterations:")
      val (patterns, nodeIdToClusterLabel) = createPatternsFromKMeansClusters(predictions, propertyDataTypes)
      // Evaluate clustering if needed
  ...
  def createEdgesFromRelationships(
    ...
    val edges = relationshipsDF.rdd.flatMap { row =>
      ...
      for {
        ...
        val startNode = Node(label = startLabel, properties = Map.empty, propertyTypeCodes = Map.empty, patternId = s"Pattern_$startLabel")
        val endNode = Node(label = endLabel, properties = Map.empty, propertyTypeCodes = Map.empty, patternId = s"Pattern_$endLabel")
